...
export default function Divider({
  ...
  if (children) {
    if (Array.isArray(children)) {
      ...
      console.error(
        "Yo, Divider.tsx here: we expect only a single child for the divider component. You gave me more. I'm just gonna use the first one.",
      );
    } else {
      firstChild = children;
    }
  ...
  return (
    <div
      className={classNames(
        className,
        "x-fill col align-center justify-center",
      )}
    >
      {type === "asym" ? (
        <AsymmetricLine color={`var(--${color})`} />
      ) : (
        <SymmetricLine color={`var(--${color})`} />
      )}
      {firstChild && (
        <div className={classNames(styles[type])}>{firstChild}</div>
      )}
    </div>
  );
}
